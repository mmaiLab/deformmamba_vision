...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=0,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                layer_input_dim = embed_dim
            else:
                layer_input_dim = int(embed_dim * 2 ** (i_layer - 1))

            layer = BasicLayer(dim=input_dim,
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            x_view = x_view.permute(0, 3, 1, 2).contiguous()  # B, C, H, W
            outputs[str(i)] = x_view
        x = self.norm(x)
        return outputs
