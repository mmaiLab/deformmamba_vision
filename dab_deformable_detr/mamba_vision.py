...
from DCNv4.functions import DCNv4Function as deformable_conv
from torch.nn.init import xavier_uniform_, constant_

class DeformableConvolution(nn.Module):
    def __init__(self, channels, group=4, offset_scale=1.0):
        super().__init__()
        assert channels % group == 0 and (channels // group) % 16 == 0
        self.offset_mask = nn.Linear(channels, int(math.ceil((group * 3 * 3 * 3)/8)*8))
        self.deformable_conv = lambda x, m: deformable_conv.apply(x, m, 3, 3, 1, 1, 1, 1, 1, 1, group, channels // group, offset_scale, 256, False)
    ...
    def forward(self, x, H, W):
        x = rearrange(x, "b d (h w) -> b h w d", h=H, w=W).contiguous()
        offset_mask = self.offset_mask(x)
        x_deform = self.deformable_conv(x, offset_mask)
        x = self.norm(x + x_deform)
...
class MambaVisionMixer(nn.Module):
    def __init__(
        ...
        print(f"DeformableConvolution(channels={self.d_inner//2}, group={self.d_inner//64})")
        self.deform_conv = DeformableConvolution(channels=self.d_inner//2, group=self.d_inner//64)
