...
class SpatialMamba(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x, x_next = layer(x)
            outputs[str(i)] = x_next.permute(0, 3, 1, 2).contiguous()
            x = x_next
...
class Backbone_SpatialMamba(SpatialMamba):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer="ln", **kwargs):
        ...
        norm_layer_func: nn.Module = _NORMLAYERS.get(norm_layer.lower(), None) 
        kwargs.update(norm_layer=norm_layer_func)       
        ...
        for i in out_indices:
            layer = norm_layer_func(self.dims[i]*2)
            if i==3:
                layer = norm_layer_func(self.dims[i])
